...
const WeatherForecast = ({ data }: WeatherForecastProps) => {
    return (
        <Card inset className="grow">
            <CardContent className="grid grid-cols-2 sm:grid-col-2 md:grid-cols-3 gap-2 lg:grid-cols-5">
                {data && data.forecast && data.forecast.forecastday ? (
                    data.forecast.forecastday.map((day, index) => {
                        ...
                        return (
                            <div
                                key={index}
                                className="p-2 text-center flex flex-col items-center grow"
                            >
                                <p>{dayOfWeek}</p>
                                <Image
